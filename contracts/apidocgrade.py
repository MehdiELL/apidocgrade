...
# ApiDocGrade - AI API-docs quality grader on GenLayer.
...
SCORE_TOLERANCE = 25
CATEGORIES = ("excellent", "adequate", "poor")
...
class ApiDocGrade(gl.Contract):
    ...
    def evaluate(self, item_id: int) -> None:
        ...
        def leader_fn():
            ...
            prompt = f"""You evaluate API documentation. Judge the quality of the API DOCS page (completeness, examples, clarity).
...
Classify the CONTENT as exactly one of: excellent, adequate, poor.
Also give an integer quality score from 0 to 100, and a one-sentence reason.

Respond with ONLY JSON: {{"label": "<one of: excellent, adequate, poor>", "score": <integer 0-100>, "reasoning": "<one sentence>"}}"""
