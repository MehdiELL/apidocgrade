...
### AI API-docs quality grader on GenLayer
...
Submit a URL; GenLayer validators independently evaluate it with an LLM, agree on a label and score by consensus, and write the result on chain. AI API-docs quality grader.

**Deployed contract (Testnet Bradbury):** [`0x461f9099A27e8505Fcbb2c37bEe0B5ECb4189414`](https://explorer-bradbury.genlayer.com/address/0x461f9099A27e8505Fcbb2c37bEe0B5ECb4189414)
...
2. **`evaluate(id)`** - each validator fetches the source, classifies the content
   (one of: excellent, adequate, poor) and scores it 0-100, then must **agree** on the label and a similar
   score via the Equivalence Principle.
3. The **label, score, and reasoning** are written on chain.

## The Intelligent Contract

`contracts/apidocgrade.py` targets the GenVM Python runner (pinned by hash):
...
genvm-lint check contracts/apidocgrade.py
